...
import {
  ...
  Building2, CirclePlus, ClipboardCheck, Clock3, Cloud, CookingPot, FolderPlus, Gauge, History, KeyRound, LayoutList,
...
function AuthScreen({ onAuthenticated }) {
  ...
  const [form, setForm] = useState({ name: '', email: '', slug: '', pin: '', statsPin: '' })
  const [error, setError] = useState('')
  const [loading, setLoading] = useState(false)
  const submit = async (event) => {
    ...
    try {
      let festival
      if (mode === 'register') {
        await sagraApi.linkOwnerEmail(form.email)
        festival = await sagraApi.registerFestival(form.name.trim(), slugify(form.slug || form.name), form.pin.trim(), form.statsPin.trim())
      } else {
        festival = await sagraApi.loginFestival(slugify(form.slug), form.pin.trim())
      }
      onAuthenticated(festival, mode === 'register' ? 'Sagra creata. Conferma l’email del proprietario.' : '')
  ...
  return <main className="auth-page"><section className="auth-card"><div className="auth-icon"><Smartphone size={32} /></div><h1>Sagra Cloud</h1><p>{mode === 'login' ? 'Inserisci il PIN per accedere' : 'Registra la tua attività'}</p><div className="auth-tabs"><button className={mode === 'login' ? 'active' : ''} onClick={() => setMode('login')}>Accedi</button><button className={mode === 'register' ? 'active' : ''} onClick={() => setMode('register')}>Nuova sagra</button></div><form onSubmit={submit}>{mode === 'register' ? <><input aria-label="Nome attività" placeholder="Nome attività" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required /><input aria-label="Email proprietario" type="email" placeholder="Email proprietario per il recupero" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} required /></> : null}<input aria-label="Codice attività" placeholder="Codice attività (es. sagra-orio)" value={form.slug} onChange={(event) => setForm({ ...form, slug: event.target.value })} required={mode === 'login'} /><input aria-label="PIN operativo" type="password" inputMode="numeric" pattern="[0-9]{4,12}" placeholder="PIN operativo (da 4 cifre)" value={form.pin} onChange={(event) => setForm({ ...form, pin: event.target.value })} minLength={4} maxLength={12} required />{mode === 'register' ? <input aria-label="PIN statistiche" type="password" inputMode="numeric" pattern="[0-9]{4,12}" placeholder="PIN statistiche diverso (da 4 cifre)" value={form.statsPin} onChange={(event) => setForm({ ...form, statsPin: event.target.value })} minLength={4} maxLength={12} required /> : null}{error ? <p className="form-error">{error}</p> : null}<button className="primary-button" disabled={loading}>{loading ? 'Attendi…' : mode === 'login' ? 'Accedi' : 'Crea attività'}</button></form>{mode === 'login' ? <button className="recovery-link" onClick={() => setMode('recover')}><KeyRound size={15} /> Hai dimenticato codice o PIN?</button> : null}{isDemoMode ? <button className="demo-link" onClick={() => setForm({ name: '', email: 'demo@sagra.local', slug: 'sagra-demo', pin: '12345', statsPin: '9999' })}>Demo: sagra-demo · PIN 12345 · statistiche 9999</button> : null}</section></main>
}

function Header({ festival, view, setView, onLogout }) {
  return <header className="app-header"><div className="header-inner">{view !== 'dashboard' ? <button className="icon-button" aria-label="Indietro" onClick={() => setView('dashboard')}><ArrowLeft size={19} /></button> : null}<div className="brand-block"><h1>Sagra Cloud</h1><p>{festival.name} · {viewTitles[view]}</p></div><div className="live-status"><span /> REALTIME</div><button className="icon-button" aria-label="Esci" onClick={onLogout}><LogOut size={18} /></button></div></header>
}
...
function HistoryScreen({ orders, onLoad }) {
  ...
  return <div className="history-wrap"><section className="panel filters"><label>Seleziona giorno archivio<input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label>Cerca tavolo<input placeholder="Numero tavolo" value={table} onChange={(event) => setTable(event.target.value)} /></label></section>{loading && !filtered.length ? <div className="loading compact"><span /></div> : !filtered.length ? <EmptyState icon={Archive} title="Nessun ordine trovato" description="Prova a cambiare giorno o numero del tavolo." /> : <><div className="history-list">{filtered.map((order) => <article key={order.id}><div><h3>Tavolo {order.table_number} <small>#{compactId(order.id)}</small></h3><p>{new Date(order.created_at).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })} · {order.paid ? 'Pagato' : 'Da pagare'}</p></div><strong>{euro(order.total)}</strong></article>)}</div>{hasMore ? <button className="load-more" disabled={loading} onClick={() => loadPage(true)}>{loading ? 'Caricamento…' : 'Carica altre 200 comande'}</button> : null}</>}</div>
}
...
function StatsScreen({ orders, onLoad }) {
...
export default function App() {
  ...
  const logout = () => { localStorage.removeItem(SESSION_KEY); sessionStorage.removeItem('sagra-stats-unlocked'); setFestival(null); setViewState('dashboard'); setData({ categories: [], products: [], orders: [], orderCount: 0, membership: null }) }
